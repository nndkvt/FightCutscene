...
namespace VFolders.Libs
{
    public static class VUtils
    {
        ...
        public static void SetKeyword(this Material mat, string kword, bool set) => mat.SetKeyword(new LocalKeyword(mat.shader, kword), set);



#if UNITY_EDITOR

        public static List<string> GetAllTextureNames(this Material material)
        {
            var l = new List<string>();
            for (int i = 0; i < ShaderUtil.GetPropertyCount(material.shader); i++)
                if (ShaderUtil.GetPropertyType(material.shader, i) == ShaderUtil.ShaderPropertyType.TexEnv)
                    l.Add(ShaderUtil.GetPropertyName(material.shader, i));

            return l;
        }
        public static List<Texture> GetAllTextures(this Material material) => material.GetAllTextureNames().Select(r => material.GetTexture(r)).ToList();

        public static Texture GetTextureIfPossible(this Material mat, string name)
        {
            if (!mat.GetAllTextureNames().Contains(name)) return null;

            return mat.GetTexture(name);
        }
        public static void SetTextureIfPossible(this Material mat, string name, Texture texture)
        {
            if (!mat.GetAllTextureNames().Contains(name) || mat.GetTexture(name) == texture) return;
