    ...
    public static class VFolders
    {
        static void RowGUI(string guid, Rect rowRect)
        {
            ...
            void setObjects()
            {
                ...
                tree_();
                treeItem_byRect();
                treeItem_byInstanceId();

            }
            void setState()
            {
                void isTreeFocused_oneColumn()
                {
                ...
                void isTreeFocused_twoColumns()
                {
                    ...
                    isTreeFocused = EditorWindow.focusedWindow == browser && tree.treeViewController.GetFieldValue<int>("m_KeyboardControlID") == GUIUtility.keyboardControl;

                }
                void isSelected_oneColumn()
                {
                    if (!curEvent.isRepaint) return;
                    if (isListArea) return;
                    if (tree == null) return;
                ...
                isTreeFocused_oneColumn();
                isTreeFocused_twoColumns();
                isSelected_oneColumn();
                isSelected_TwoColumns();
                lastHovered_();

                lastKnownMousePosition_screenSpace = curEvent.mousePosition_screenSpace;
                isRowBeingRenamed = isTreeFocused && isRowSelected && EditorGUIUtility.editingTextField;
            ...
            void drawing()
            {
                ...
                var showDefaultIcon = !showCustomIcon && !useMinimalMode;
                ...
                void hierarchyLines()
                {
                    ...
                    bool isLastChild(TreeViewItem item) => item.parent?.children[item.parent.children.Count - 1] == item;
        ...
        static void ProjectBrowserItemGUI(string guid, Rect rect)
        {
            ...
            if (foldersFirstNeedsUpdateFromGUI)
                UpdateFoldersFirst();
        ...
        class Tree
        {
            ...
            public void DelayCallUpdateLoop()
            {
                ...
                if (!Application.isPlaying)
                    expandedIds = treeViewController.GetPropertyValue("state").GetPropertyValue<List<int>>("expandedIDs");


                EditorApplication.delayCall -= DelayCallUpdateLoop;
                EditorApplication.delayCall += DelayCallUpdateLoop;
        ...
        {
            foldersFirstNeedsUpdateFromGUI = false;
            ...
            foreach (var browser in allProjectBrowsers)
            {
                ...
                void twoColumns()
                {
                    ...
                    if (foldersFirstInitedForBrowsersInTwoColumnMode.Contains(browser)) return;

                    var m_ListArea = browser.GetFieldValue("m_ListArea");

                    if (m_ListArea == null) return;

                    m_ListArea.SetPropertyValue("foldersFirst", true);  // todo investigate foldersOnly for vtabs one column
                    browser.InvokeMethod("InitListArea");

                    foldersFirstInitedForBrowsersInTwoColumnMode.Add(browser);
                    foldersFirstInitedForBrowsersInOneColumnMode.Remove(browser);

                }
                void oneColumn()
                {
                    if (isTwoColumns) return;
                    if (foldersFirstInitedForBrowsersInOneColumnMode.Contains(browser)) return;
                    ...
                    foldersFirstInitedForBrowsersInOneColumnMode.Add(browser);
                    foldersFirstInitedForBrowsersInTwoColumnMode.Remove(browser);
                }
        ...
        static List<Object> foldersFirstInitedForBrowsersInOneColumnMode = new List<Object>();
        static List<Object> foldersFirstInitedForBrowsersInTwoColumnMode = new List<Object>();

        static bool foldersFirstNeedsUpdateFromGUI = true;
